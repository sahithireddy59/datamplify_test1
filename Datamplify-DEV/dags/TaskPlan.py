...
def check_for_dynamic_file(path):
    ...
    """
    pattern = f"/var/www/AB_Client/{path.strip()}"
    ...
    if task_type == 'taskCommand':
        if task_conf['command_type'].lower() == 'external':
            ...
            task = PythonOperator(
                ...
                op_kwargs={"commands": full_command,"parent_task_id":parent_task_id},
            )
        ...
        task = EmailOperator(
            task_id=task_id,
            from_email="Support Analytify <support@analytify.ai>",
            ...
            custom_headers={"Reply-To": "noreply@analytify.com"}
        )
...
def fetch_and_lock_dag_configs(limit=100):
    base_dir = 'Airflow/Configs/FlowBoard'
    with connection.cursor() as cursor:
        cursor.execute("""                       
            ...
            SET "parsed" = NOW()
            FROM to_parse
            WHERE "TaskPlan"."Task_id" = to_parse."Task_id"
            RETURNING "TaskPlan"."Task_id";
        """, [limit])
        
        rows = cursor.fetchall() 
        for file_id in rows:
            full_path = os.path.join(base_dir, f'{file_id}.json')
            try:
                with open(full_path) as f:
                    config_json = json.load(f)
                    yield dag_id, config_json
            except Exception as e:
                print(f"[ERROR] Failed to parse {full_path}: {e}") # List of tuples
